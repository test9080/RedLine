...
- (NSString *)groupTimeIntervalDescription
{
    NSTimeInterval timeInterval = -[self timeIntervalSinceNow];
    ...
    } else if (timeInterval < D_WEEK) {// 7天内
...
- (NSString *)lifeTimeString:(NSString *)foramtString {
    NSDateFormatter *dateFormatter = [NSDateFormatter dateFormatterWithFormat:foramtString];

    NSTimeInterval timeInterval = -[self timeIntervalSinceNow];
    if (timeInterval < 60) {
        return @"刚刚";
    } else if (timeInterval < 3600) { // 1小时以内
        return [NSString stringWithFormat:@"%.f分钟前", timeInterval / 60];
    } else if (timeInterval < 86400) { // 24小时以内
        return [NSString stringWithFormat:@"%.f小时前", timeInterval / 3600];
    } else if (timeInterval < 172800) {// 24-48小时
        [dateFormatter setDateFormat:@"HH:mm"];
        return [NSString stringWithFormat:@"昨天%@", [dateFormatter stringFromDate:self]];
    } else if (timeInterval < 259200) {// 3天内
        return [NSString stringWithFormat:@"%.f天前", timeInterval / 86400];
    } else if (timeInterval < 31536000) {//3天至1年内
        [dateFormatter setDateFormat:foramtString];
        return [dateFormatter stringFromDate:self];
    } else {
        return [NSString stringWithFormat:@"%.f年前", timeInterval / 31536000];
    }
...
/**个人爱生活定制 - 距离当前的时间间隔描述*/
- (NSString *)myLifeTimeIntervalDescription
{
    ...
    //        return @"1分钟内";
    //    } else
    // 产品发话了: 当天发表的也显示日期 不显示 具体 时间了
//    if (timeInterval < 86400) { // 24小时以内
//        [dateFormatter setDateFormat:@"HH:mm"];
//        return [dateFormatter stringFromDate:self];
//    } else
        if (timeInterval < 31536000) {//3天至1年内
...
/*广告的推送时间*/
- (NSString *)advertingformattedTime:(NSString *)foramtString{
    NSDateFormatter *formatter = [NSDateFormatter dateFormatterWithFormat:foramtString];
    [formatter setDateFormat:@"yyyyMMdd"];
    NSString * dateNow = [formatter stringFromDate:[NSDate date]];
    NSDateComponents *components = [[NSDateComponents alloc] init];
    [components setDay:[[dateNow substringWithRange:NSMakeRange(6,2)] intValue]];
